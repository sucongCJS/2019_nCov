...
def count(initial, data, row, col):
    ...
    if initial[row, col] == State.infected:
        data[row, col] = np.random.choice(
                ...
                p=[recovery_rate, 1-recovery_rate])
    # 如果是易感的
...
def generate(frame_num, img, plt, initial):
    ...
    img.set_data(data)
    initial[:] = data[:]
    print(data[length//2, length//2])
    return img

# 制作动图
def update(data): 
    update_interval = 1000 #更新频率，以ms计
    fig, ax = plt.subplots()
    ax.set_xticks([])
    ax.set_yticks([])
    img = ax.imshow(data, cmap='Blues', interpolation='none')
...
if __name__ == "__main__":
    transmission_rate = 1 # 感染概率0~1
    recovery_rate = 0.00 # 康复率0~1
    spontaneous_activation_rate = 0.1
    length = 6 # 宽度
    main()